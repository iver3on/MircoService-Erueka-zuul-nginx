...
在服务网关中定义过滤器只需要继承ZuulFilter抽象类实现其定义的四个
抽象函数就可对请求进行拦截与过滤。 
 */
...
public class AccessFilter extends ZuulFilter  {
    ...
    @Override
    public Object run() {
    	/*，定义了一个Zuul过滤器，实现了在请求被路由之前检查请求中是否有accessToken参数，若有就进行路由，若没有就拒绝访问，
        ...
        if(accessToken == null) {
            ...
            //通过ctx.setSendZuulResponse(false)令zuul过滤该请求，不对其路由
            ctx.setSendZuulResponse(false);
            //通过ctx.setResponseStatusCode(401)设置了其返回的错误码
            ctx.setResponseStatusCode(401);
            ctx.getResponse().setContentType("text/html;charset=UTF-8");
            ctx.setResponseBody("您没有权限啊！");
            return null;
        }
